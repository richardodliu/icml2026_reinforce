...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.77\linewidth]{imgs/orm_reward_kl.pdf}
    \caption{Comparison of smoothed Training Reward and KL Divergence. \ours ($k=1$) achieves strong rewards with significantly more stable (lower) KL divergence, illustrating a different reward--KL trade-off.}
...
\begin{table}[!t]
    \small
    \centering
\caption{Comparison on Chat-Arena-Hard \citep{li2024crowdsourced}. The single-sample \ours ($k=1$) achieves a top-tier score with a higher score/length ratio than group-sampling GRPO.}
...
As shown in \Cref{tab:general_exp}, the single-sample \textbf{\ours ($k=1$)} achieves a score of 46.7, close to the group-sampling GRPO (46.8). However, it produces shorter responses (832 tokens vs. 860), yielding a higher score/length ratio (0.0561). This result supports competitive alignment with single-response sampling; score/length is not a direct measure of training compute efficiency.

\paragraph{Results Analysis}
\Cref{fig:orm_reward_kl} shows the training dynamics. GRPO's reward and KL divergence rise rapidly, while \ours exhibits a more gradual reward increase and smaller divergence. These curves describe different reward--KL trade-offs; they do not alone establish reward hacking. Response length can also influence preference-based evaluation~\citep{dubois2024length}. We therefore report both evaluation scores and generated lengths rather than equating higher training reward with better generalization.
...
To test for overfitting, we trained on only 30 questions from AIME-24 and evaluated on AIME-25. As shown in \Cref{tab:small_exp}, GRPO (local norm) achieves a near-perfect 95.0\% on the training set but scores only 0.0 Pass@1 and 0.4 Pass@16 on the test set, indicating severe overfitting. In contrast, \ours with global norm, while scoring lower on the training set (71.0), shows higher held-out scores (2.5 Pass@1, 40.0 Pass@16). % <-- MODIFIED: User changed this from Baseline.

\begin{table}[!t]
    \small
    \centering
\caption{Comparison on a small training dataset. GRPO (local norm) shows a large train--test gap, while \ours (global norm) generalizes.}
    \label{tab:small_exp}
    \begin{tabular}{l|ccc}
    \toprule
...
The curves show the same overfitting pattern: GRPO fits the training questions quickly, while \ours learns more gradually. % <-- MODIFIED: User changed this from Baseline.
...
\end{minipage}
\captionof{figure}{Training curves on a small prompt dataset. Left: GRPO (local norm) immediately overfits. Right: \ours (global norm) learns more stably.}
\label{fig:details}
\end{center}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=0.53\linewidth]{imgs/logic_score.pdf}
...
As shown in \Cref{tab:performance_comparison_main}, \varn achieves the highest average accuracy (24.10) across all benchmarks. It outperforms both GRPO (22.58) and the full-critic PPO (21.85). It demonstrates that our stable, critic-free approach is highly effective for complex agentic tasks, surpassing even the heavyweight PPO algorithm. The combined recipe leads on four of five tasks; this comparison does not isolate the individual effects of reward centering, normalization, and KL regularization.
