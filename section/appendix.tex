...
We are training with samples from $\pi_{\theta}$, so we are approximating the \textbf{Reverse KL (RKL)}, $D_{KL}(\pi_{\theta}||\pi_{ref})$. The practical policy gradient for RKL is (more details are in \citep{liu2025rethinking}):
$$
...
We can \textbf{exclude k1} as a loss term. Its gradient does not depend on $\pi_{ref}$ and thus provides no constraining effect. The loss $\mathcal{J}_{k{1}} = \mathbb{E}_{y\sim\pi_{\theta}}[\log \pi_{\theta} - \log \pi_{ref}]$ differentiates to $\nabla_{\theta}\mathcal{J}_{k{1}} = \mathbb{E}_{y\sim\pi_{\theta}}[\nabla_{\theta}\log \pi_{\theta}]$, as the $\pi_{ref}$ term vanishes. (Note: k1 \emph{is} used inside the reward for REINFORCE++, but not as a separate loss term).

\paragraph{$k_2$ Analysis}
The $k_2$ estimator provides a gradient that is \textbf{equivalent to the theoretical Reverse KL gradient}.
...
The $k_3$ gradient is problematic. It is theoretically an estimator for the \textbf{Forward KL} ($KL(\pi_{ref}||\pi_{\theta})$), not the Reverse KL. There is a mismatch: we are sampling from $\pi_{\theta}$, but the Forward KL gradient requires sampling from $\pi_{ref}$.
